...
player_hit = False  # Initialize the player_hit flag
...
while running:
    ...
    for i in range(num_of_enemies):
        if not player_hit and abs(enemyY[i] - playerY) < 70 and abs(enemyX[i] - playerX) < 70:
            game_over = True
            break  # exit the loop immediately when game_over is True
    ...
    if game_over:
        screen.blit(game_over_text, ((displayresolutionX / 2)
                    * 0.7, (displayresolutionY / 2) * 0.8))
